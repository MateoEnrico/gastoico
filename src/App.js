...
import { InicioSesion } from "./components/UI/Inicio-sesion.jsx";
...
function App() {
  ...
  // Cálculo de Trial (30 días)
  const isReadOnly = (() => {
    if (!session?.user?.created_at) return false;
    const createdAt = new Date(session.user.created_at);
    const msDiff = Date.now() - createdAt.getTime();
    const daysDiff = msDiff / (1000 * 60 * 60 * 24);
    return daysDiff > 30; // Después de 30 días, bloquea
  })();
    ...
    <div className="cuerpo">
      {isReadOnly && (
        <div className="trial-banner">
          ...
          <span>Estás en modo solo-lectura. Renová tu plan para seguir calculando tus costos.</span>
        </div>
      )}
      ...
      <ParticleCanvas />
      <Navbar session={session} onShowLogin={() => setShowLoginModal(true)} />

      {showLoginModal && <InicioSesion onClose={() => setShowLoginModal(false)} />}
      ...
      <div className="contenido">
        <section id="productos">
          <Producto
            ...
            onRequireLogin={() => {
              if (!session) { setShowLoginModal(true); return true; }
              return false;
            }}
          />
        </section>
        <section id="materia-prima">
          <MateriaPrima
            ...
            isReadOnly={isReadOnly}
            onRequireLogin={() => { if (!session) { setShowLoginModal(true); return true; } return false; }}
          />
        </section>
        ...
        <section id="impuestos">
          <Impuestos
            impuestos={impuestos} setImpuestos={setImpuestos}
            isReadOnly={isReadOnly}
            onRequireLogin={() => { if (!session) { setShowLoginModal(true); return true; } return false; }}
          />
        </section>
